...
Tablet editing is reducer-owned and repository-persisted. `apps/mobile/src/workspace/mobileWorkspaceEditing.ts` applies note creation, title/body edits, frontmatter scalar property changes, relationship add/remove, archive, favorite, saved-view definition changes, and Type definition metadata changes against the current `MobileWorkspaceSnapshot`. Saved-view edits persist desktop YAML fields including filters, `order`, and `listPropertiesDisplay`, so mobile view ordering and note-list column choices use the same file contract as desktop. Type section edits persist the Type markdown document frontmatter for sidebar label, visibility, color, sort, and note-list display properties through `apps/mobile/src/workspace/mobileTypeDefinitions.ts`. The reducer preserves frontmatter while the markdown editor edits body content, derives snippets/blocks/links back from Markdown, resolves wikilink relationship targets against the snapshot, and returns write plans. Mobile note snapshots preserve frontmatter aliases so wikilink resolution and editor autocomplete can match desktop identity candidates. The repository boundary decides whether those writes are no-ops for fixtures, host-injected QA logs, or native Expo filesystem writes. Visual components do not import filesystem APIs directly.
